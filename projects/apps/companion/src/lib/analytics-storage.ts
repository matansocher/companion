...
export async function getDailyTotals(days: number): Promise<{ date: string; total: number }[]> {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const dayTotal = allVisits.filter((v) => v.startTime >= dayStart && v.startTime < dayEnd).reduce((sum, v) => sum + v.duration, 0);
...
export function getBudgetStatus(domain: string, todayDurationMs: number, budgets: FocusBudget[]): { status: BudgetStatus; budget: FocusBudget | null; usagePercent: number } {
...
export async function exportData(format: 'csv' | 'json'): Promise<string> {
  ...
  const rows = visits
    .map((v) => {
      ...
      return [escapeCsv(v.id), escapeCsv(v.url), escapeCsv(v.domain), escapeCsv(v.title), v.startTime, v.endTime, v.duration].join(',');
    })
    .join('\n');
