...
export function TrendChart({ data }: TrendChartProps) {
  ...
  return (
    <div className="rounded-xl bg-secondary p-4">
      <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Daily Trend</h3>
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="group flex flex-1 flex-col items-center gap-1">
              ...
              <span className="text-[9px] text-muted-foreground truncate w-full text-center">{d.date.split(',')[0].split(' ')[0]}</span>
            </div>
          );
        })}
      </div>
