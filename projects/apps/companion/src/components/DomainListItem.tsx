import type { DomainSummary, BudgetStatus } from "@companion/shared"
import { formatDuration } from "../lib/analytics-storage"
import { ChevronRight, AlertTriangle, OctagonX, Pin } from "lucide-react"
  ...
  return (
    ...
    >
      <div className="relative">
        <img
          src={`https://www.google.com/s2/favicons?domain=${summary.domain}&sz=32`}
          alt=""
        ...
        {budgetStatus === "warning" && (
          <AlertTriangle className="absolute -right-1 -top-1 h-3.5 w-3.5 text-amber-500" />
        )}
        {budgetStatus === "exceeded" && (
          <OctagonX className="absolute -right-1 -top-1 h-3.5 w-3.5 text-red-500" />
        )}
      </div>
      <div className="flex-1 text-left min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {isActive && (
            <span className="shrink-0 rounded-full bg-green-500/20 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-green-500">
              Live
            </span>
          )}
          <span
            className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${categoryColors[summary.category] || categoryColors.Other}`}
          >
            {summary.category}
          </span>
        </div>
        <div className="mt-0.5 flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          {budgetStatus !== "ok" && (
            <span className={cn(
              "tabular-nums",
              budgetStatus === "warning" ? "text-amber-500" : "text-red-500"
            )}>
