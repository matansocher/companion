import { ArrowLeft, Clock, Plus, Target, X } from 'lucide-react';
...
export function FocusSettings({ budgets, onSave, onBack }: FocusSettingsProps) {
  ...
  const handleUpdateLimit = (index: number, newHours: number) => {
    const updated = budgets.map((b, i) => (i === index ? { ...b, dailyLimitMs: newHours * 60 * 60 * 1000 } : b));
    onSave(updated);
  };
    ...
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="rounded-xl bg-secondary p-4">
          <p className="text-xs text-muted-foreground">Set a daily time budget for specific domains. A yellow warning appears at 80% usage and a red warning when the budget is exceeded.</p>
        </div>
              ...
              {PRESET_HOURS.map((h) => (
                <button
                  ...
                  className={cn(
                    ...
                    hours === h ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground',
                  )}
                >
        ...
        {budgets.length > 0 && (
          <div className="space-y-2">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Active Budgets ({budgets.length})</h3>
            {budgets.map((budget, index) => (
              <div key={budget.domain} className="rounded-xl bg-secondary p-4">
                <div className="flex items-center gap-3">
                  <img src={`https://www.google.com/s2/favicons?domain=${budget.domain}&sz=32`} alt="" className="h-5 w-5 rounded" />
                  <span className="flex-1 text-sm font-medium text-foreground truncate">{budget.domain}</span>
                  <button
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 border-t border-border pt-3">
                    {PRESET_HOURS.map((h) => (
                      <button
                        ...
                        className={cn(
                          'rounded-lg px-3 py-1.5 text-xs font-medium transition-colors',
                          budget.dailyLimitMs === h * 60 * 60 * 1000 ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground',
                        )}
                      >
                        {formatDuration(h * 60 * 60 * 1000)}
                      </button>
        ...
        {budgets.length === 0 && (
          <div className="flex flex-col items-center py-8 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Add a domain above to start tracking limits</p>
          </div>
        )}
      </div>
    </div>
