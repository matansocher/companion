...
import { getTelegramChats, getTelegramMessages } from '../lib/telegram';
import { cn } from '../lib/utils';
import { Skeleton } from './ui/skeleton';

type TelegramView = { screen: 'list' } | { screen: 'chat'; chat: TelegramChat };

type ListState = { status: 'loading' } | { status: 'no_tab' } | { status: 'error'; message: string } | { status: 'ok'; chats: TelegramChat[] };

type ChatState = { status: 'loading' } | { status: 'error'; message: string } | { status: 'ok'; messages: TelegramMessage[] };

export function Telegram() {
  const [view, setView] = useState<TelegramView>({ screen: 'list' });
...
function ChatListView({ onSelectChat }: { onSelectChat: (chat: TelegramChat) => void }) {
  const [state, setState] = useState<ListState>({ status: 'loading' });

  const loadChats = useCallback(async () => {
    setState({ status: 'loading' });
    const result = await getTelegramChats();
  ...
  if (state.status === 'no_tab') {
    return (
      <div className="flex flex-1 flex-col items-center justify-center px-4 text-center">
        ...
        <h2 className="mb-2 text-lg font-medium text-foreground">Telegram Not Open</h2>
        <p className="mb-4 text-sm text-muted-foreground">
          Open{' '}
          <a href="https://web.telegram.org" target="_blank" rel="noreferrer" className="text-primary underline">
            web.telegram.org
          </a>{' '}
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        {chats.map((chat, i) => (
          <ChatRow key={`${chat.peerId || chat.name}-${i}`} chat={chat} onClick={() => onSelectChat(chat)} />
        ))}
      </div>
    </div>
  );
...
function ChatRow({ chat, onClick }: { chat: TelegramChat; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex w-full items-center gap-3 border-b border-border/50 px-4 py-2.5 text-left transition-colors hover:bg-muted/30">
      ...
      {chat.avatarUrl ? (
        ...
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/20 text-sm font-semibold text-primary">{chat.avatarText}</div>
      )}
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            {chat.unreadCount > 0 && (
              <span className={cn('flex h-4.5 min-w-4.5 items-center justify-center rounded-full px-1 text-[10px] font-bold text-white', chat.isMuted ? 'bg-muted-foreground/50' : 'bg-primary')}>
                {chat.unreadCount}
              </span>
            )}
          </div>
...
function ConversationView({ chat, onBack }: { chat: TelegramChat; onBack: () => void }) {
  const [state, setState] = useState<ChatState>({ status: 'loading' });
  const bottomRef = useRef<HTMLDivElement>(null);

  const loadMessages = useCallback(async () => {
    setState({ status: 'loading' });
    const result = await getTelegramMessages(chat.peerId);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-3 border-b border-border px-3 py-2">
        ...
        {chat.avatarUrl ? (
          ...
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/20 text-xs font-semibold text-primary">{chat.avatarText}</div>
        )}
...
function MessageBubble({ message }: { message: TelegramMessage }) {
  return (
    <div className={cn('mb-1.5 flex', message.isOwn ? 'justify-end' : 'justify-start')}>
      <div className={cn('max-w-[80%] rounded-2xl px-3 py-1.5', message.isOwn ? 'rounded-br-sm bg-primary/20' : 'rounded-bl-sm bg-secondary')}>
        {message.senderName && !message.isOwn && <p className="mb-0.5 text-[10px] font-semibold text-primary">{message.senderName}</p>}
