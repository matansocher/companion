...
import type { TimeRange, DomainSummary, FocusBudget, AnalyticsSettings } from "@companion/shared"
import { getDomainSummaries, getDailyTotals, formatDuration, categorizeDomain, getFocusBudgets, getTodayDomainDurations, getAnalyticsSettings } from "../lib/analytics-storage"
...
export function Analytics({
  ...
  const loadData = useCallback(async () => {
    ...
    const [sums, daily, b, today] = await Promise.all([
      getDomainSummaries(timeRange, settings.customCategories),
      getDailyTotals(7),
      getFocusBudgets(),
      getTodayDomainDurations(),
    ])
    setSummaries(sums)
    setDailyData(daily)
    setBudgets(b)
  ...
  const filteredSummaries = useMemo(() => {
    ...
    return sortedSummaries.filter(
      (s) => s.domain.toLowerCase().includes(q) || s.category.toLowerCase().includes(q)
    )
          ...
          <div>
            <div className="mb-2 flex items-center gap-2">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                {searchQuery ? `Results for "${searchQuery}"` : "Top Sites"}
              </h3>
            </div>
  ...
  if (exists) {
    return summaries.map((s) =>
      s.domain === activeDomain
        ? { ...s, totalDuration: s.totalDuration + liveElapsed }
        : s
    )
  }
