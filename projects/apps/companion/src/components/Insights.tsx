...
import type { DomainCategory, TimeRange } from '@companion/shared';
import { type CategoryBreakdown, formatDuration, getAnalyticsSettings, getCategoryBreakdown, getHeatmapData, getTabCounts, type HeatmapData, type TabCountEntry } from '../lib/analytics-storage';
...
export function Insights({ dailyData = [] }: InsightsProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    const [cats, heat, tabs] = await Promise.all([getCategoryBreakdown(timeRange, settings.customCategories), getHeatmapData(timeRange), getTabCounts(timeRange)]);
    setCategories(cats);
    setHeatmap(heat);
    setTabCounts(tabs);
  }, [timeRange]);
  ...
  if (data.length === 0) {
    return (
      <div className="rounded-xl bg-secondary p-3 text-center">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Categories</h3>
        ...
        <div className="w-full space-y-1">
          {segments.map((seg) => (
            <div key={seg.category} className="flex items-center gap-1.5">
              <span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: CATEGORY_COLORS[seg.category] }} />
              <span className="flex-1 text-[10px] text-foreground truncate">{seg.category}</span>
              <span className="text-[9px] tabular-nums text-muted-foreground">{Math.round(seg.percentage * 100)}%</span>
            </div>
          ))}
        </div>
      </div>
...
function HeatmapChart({ data }: { data: HeatmapData }) {
  if (data.length === 0) {
    return (
      <div className="rounded-xl bg-secondary p-4 text-center">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Activity Heatmap</h3>
  ...
  return (
    <div className="rounded-xl bg-secondary p-4">
      <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Activity Heatmap</h3>
        ...
        <div style={{ minWidth: 320 }}>
          ...
          <div className="flex items-center justify-end gap-1 mt-2">
            ...
            {[0.1, 0.3, 0.5, 0.7, 1].map((o, i) => (
              <div key={i} className="rounded-sm" style={{ width: 10, height: 10, backgroundColor: 'rgb(59, 130, 246)', opacity: o }} />
            ))}
...
function TabCountChart({ data }: { data: TabCountEntry[] }) {
  ...
  if (data.length < 2) {
    return (
      <div className="rounded-xl bg-secondary p-3 text-center">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Open Tabs</h3>
        <p className="py-4 text-[11px] text-muted-foreground">{data.length === 0 ? 'No data yet' : 'Need more data'}</p>
      </div>
    );
  }
  ...
  const sampled = data.length > 80 ? data.filter((_, i) => i % Math.ceil(data.length / 80) === 0 || i === data.length - 1) : data;
  ...
  return (
    <div className="rounded-xl bg-secondary p-3" ref={containerRef}>
      ...
      <div className="relative overflow-hidden">
        <svg viewBox={`-4 -4 ${chartWidth + 8} ${chartHeight + 8}`} className="w-full" style={{ height: chartHeight + 8 }} onMouseMove={handleMouseMove} onMouseLeave={() => setHover(null)}>
          {/* Grid lines */}
          {[0, 0.5, 1].map((frac) => (
            <line
              key={frac}
          ...
          {/* Line */}
          <polyline points={polyline} fill="none" stroke="#a855f7" strokeWidth={1.5} strokeLinejoin="round" strokeLinecap="round" />

          {/* Hover indicator */}
          {hover && (
            <>
              <line x1={hover.x} y1={0} x2={hover.x} y2={chartHeight} stroke="#a855f7" strokeWidth={0.75} strokeDasharray="3,2" opacity={0.6} />
